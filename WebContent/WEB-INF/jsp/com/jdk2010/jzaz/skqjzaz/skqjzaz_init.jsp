...
<div class="place"> <span>位置：</span>
  <ul class="placeul">
    <li><a href="#">首页</a></li>
    <li><a href="#">建筑安装项目管理</a></li>
  </ul>
</div>
<div class="rightinfo">
  <div class="formtitle1"> <span>建筑安装项目管理</span> </div>
	...
	//var nsrsbh = document.getElementById('nsrsbh').value;
		...
		new CAPXOUS.AutoComplete("nsrsbh", function() {
			if(this.text.value.length>=3){
        		return "${ contextpath}/skqjzaz/hqmc?keyword=" + this.text.value;
			}
    	});
	
</script>
      <input name="button" type="button" class="btn" value=" 确定" onclick="toBdcList();"/>
    </li>
  </form>
</div>
</body>
...
function toBdcList(){
	...
	if(nsrsbh==''){
		...
		jQuery.ajax({
			url:"${ contextpath}/skqjzaz/checkNsrsbh?nsrsbh="+nsrsbh,
			type: "POST",
			success: function(data){
				// 提交表单成功后，释放hold，如果不释放hold，就变成了只能提交一次的表单
				if(data.status=='success'){
					window.location.href = '${ contextpath}/skqjzaz/list.htm?nsrwjbm='+data.data.nsrwjbm;					 
				}else{
					sAlert("未查询到纳税人信息");
				}
