...
<body>
	...
	<div class="formbody">
		<div id="usual1" class="usual">
			<div class="itab">
				<ul>
					<li><a href="#tab1" class="selected">机器型号编辑</a></li>
				</ul>
			</div>
			<div id="tab1" class="tabson">
				<ul class="forminfo">
					<form action="" method="POST" id="skqJqxhForm">
						...
						<input type="hidden" class="dfinput" id="id" name="skqJqxh.id"
							placeholder="请输入id" value="${ skqJqxh.id}" />
						<li><label>机器型号编码<b></b></label> <input type="text"
							class="dfinput" id="jqxhbm" name="skqJqxh.jqxhbm"
							placeholder="请输入机器型号编码" value="${ skqJqxh.jqxhbm}" /></li>
						<li><label>机器型号名称<b></b></label> <input type="text"
							class="dfinput" id="jqxhmc" name="skqJqxh.jqxhmc"
							placeholder="请输入机器型号名称" value="${ skqJqxh.jqxhmc}" /></li>
						<li><label>生产厂商<b></b></label> <input type="text"
							class="dfinput" id="sccs" name="skqJqxh.sccs"
							placeholder="请输入生产厂商" value="${ skqJqxh.sccs}" /></li>
						<li><label>状态<b></b></label>
							<div class="vocation">
								<select name="skqJqxh.status" class="select1">
									<option value="0"
										<c:if test="${ skqJqxh.status==0}">selected</c:if>>停用</option>
									<option value="1"
										<c:if test="${ skqJqxh.status==1}">selected</c:if>>启用</option>
								</select>
							</div></li>
								...
								.validator(
										{
											...
											valid : function(form) {
												...
												$
														.ajax({
															url : "${ contextpath}/skqjqxh/modifyaction",
															data : $(form)
																	.serialize(),
															type : "POST",
															success : function(
